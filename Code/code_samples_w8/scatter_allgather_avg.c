...
int main(int argc, char** argv) {
	...
	float *array=NULL, *array_chunk, *all_chunk_averages;
	...
	/* Gather all chunk averages to all other processes */
	all_chunk_averages = malloc(comm_size * sizeof(float));	
	MPI_Allgather(&local_avg_chunk, 1, MPI_FLOAT, 
	              all_chunk_averages, 1, MPI_FLOAT, MPI_COMM_WORLD);
